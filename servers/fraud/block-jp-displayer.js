...
program
    ...
    .run(async function script() {
        ...
        const z = cfg.cloudflare.zones['tgp.cash']
        ...
        let ssh = await new SSHClient().connect({username: 'root', host: cfg.getHost('belgium-mga-lb1').ip})
        for(let cmd of conditions) {
            let logs = await ssh.exec(cmd, {silent: true})
            for (let line of logs.split('\n')) {
                let [cnt, ip] = line.trim().split(' ')
                if (parseInt(cnt) > SETTINGS_THRESHOLD) {
        ...
        if(ips.length) {
            ...
            for (let ip of ips) {
                if (bannedIps.includes(ip)) {
                    ...
                    await program.chat.message(`Found "JP Displayer" bot! Banning ${ip} for tgp.cash domain`)
