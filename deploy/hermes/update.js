...
program.run(async () => {
    ...
    const allOperators = program.params.operators.split(',').map(o => cfg.operators[o])
    let locations = new Set() // stores only uniques
    for(let operator of allOperators){
        if(operator.freeze) throw Error(`Can not deploy to ${name} because the operator is frozen!`)
        locations.add(operator.location)
    }
    locations = Object.keys(cfg.locations).filter(l => locations.has(l)) // sort by the order in configurator
    ...
    for(let location of locations){
        ...
        await chat.message(`✓ ${to} deployed to ${operatorsInLocation.map(o => o.name)}`)
...
.then(async() => {
    ...
    if(changes.trim()) {
        let answer = await program.ask(`Do you want to hard reset?`, ['yes', 'no'])
        if (answer === 'yes') {
