...
program
    .description('Pre-deployment tests for SSL-Framework')
    .example(`
    ...
    .run(async () => {
        ...
        it('should be with Status of the service -> Active: active(running)', async () => {
            contains(await web1.exec(`systemctl status ssl-server-framework.service | grep "Active: active (running)"`), "Active: active (running)")
        })
