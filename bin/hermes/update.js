...
program
    ...
    .run(async (operator) => {
        ...
        if(STRATEGY === 'direct') {
            ...
            let web1 = await program.ssh(location.hosts.web1, 'dopamine')
            let lb = await program.ssh(location.hosts.lb, 'root')
            ...
            if (!otherBlueWebs.length) {
                ...
                await web1.exec(`$HOME/bin/webs-sync ${DEST} --webs${otherBlueWebs}`, {silent: true})
            }
            ...
            await web1.exec(`$HOME/bin/webs-sync ${DEST} --webs${location.green}`, {silent: true})
